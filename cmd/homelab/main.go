...
func (r *Render) Run() error {
	if r.All {
		...
		for _, entry := range entries {
			...
			if err := r.render(filepath.Join(r.SourceRoot, entry.Name()), filepath.Join(r.OutputRoot, entry.Name())); err != nil { return err }
		}
		return nil
	}
	if r.Path == "" || r.Output == "" { return fmt.Errorf("--path requires --output") }
	return r.render(r.Path, r.Output)
}

func (r *Render) render(source, output string) error {
	if _, err := os.Stat(filepath.Join(source, "kustomization.yaml")); err != nil { return fmt.Errorf("validate source %s: %w", source, err) }
	tmp, err := os.MkdirTemp("", "homelab-render-")
	if err != nil { return err }
	defer os.RemoveAll(tmp)
	if err := copyAndInterpolate(source, tmp); err != nil { return err }
	build := exec.Command(r.Kustomize, "build", "--enable-helm", tmp)
	manifest, err := build.Output()
	if err != nil { return fmt.Errorf("kustomize build %s: %w", source, err) }
	staging := output + ".staging"
	if err := os.RemoveAll(staging); err != nil { return err }
	if err := os.MkdirAll(staging, 0755); err != nil { return err }
	yq := exec.Command(r.YQ, "-s", `"`+staging+`/" + (.kind | downcase) + "_" + (.metadata.name | sub("\\.", "-"))`)
	yq.Stdin = bytes.NewReader(manifest)
	if output, err := yq.CombinedOutput(); err != nil { return fmt.Errorf("split rendered manifests: %s: %w", strings.TrimSpace(string(output)), err) }
...
func copyAndInterpolate(source, destination string) error {
	return filepath.WalkDir(source, func(path string, entry os.DirEntry, err error) error {
		...
		for _, name := range []string{"ARGOCD_GITHUB_REPO", "ARGOCD_GITHUB_ORG", "VAULT", "ARGOCD_ADMIN_GITHUB_USER"} {
			value := os.Getenv(name); if value == "" { value = map[string]string{"ARGOCD_GITHUB_REPO":"https://github.com/nandiheath/homelab-public.git", "ARGOCD_GITHUB_ORG":"https://github.com/nandiheath", "VAULT":"homelab", "ARGOCD_ADMIN_GITHUB_USER":"nandiheath"}[name] }
